...
실습
사이킷런에 존재하는 데이터를 불러오고, 불러온 데이터를 학습용, 테스트용 데이터로 분리하여 반환하는 load_data() 함수를 구현합니다.
K-fold 교차 검증을 통한 모델 학습 및 예측 수행을 진행하는 kfold_regression() 함수를 구현합니다.
실행 버튼을 눌러 검증 데이터에 대한 평균 평가 점수를 확인합니다.
Tips!
